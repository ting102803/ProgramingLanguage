...
class BasicPaser(object):
    ...
    def _create_node(self, token):
        ...
        if token is None:
            ...
            if vartable.has_key(token.lexeme) & (definevalue == 0):
                return Node(vartable[token.lexeme].type, vartable[token.lexeme].value)
            else:
...
def run_func(op_code_node):
    ...
    def define(node):
        ...
        l_node = str(node.value.next.value)
        r_node = node.value.next.next
        if r_node.type ==5:
            if r_node.value.type == 21:
                new_r_node = r_node
            else:
                new_r_node = run_expr(r_node)
        else :
            new_r_node = run_expr(r_node)
        if vartable.has_key(new_r_node.value):
            new_r_node = Node(vartable[new_r_node.value].type, vartable[new_r_node.value].value)
        vartable[l_node] = Node(new_r_node.type, new_r_node.value)
        revartable = copy.deepcopy(vartable)
        return new_r_node

    def lambdas(node):
        global vartable, revartable
        stnode = node.value
        var_node = stnode.next
        if vartable.has_key(var_node.value):
            var_node.type = vartable[var_node.value].type
            var_node.value = vartable[var_node.value].value
        var_cnode = var_node
        l_node = stnode.value.next.value
        while True:
            if vartable.has_key(str(l_node.value))==False:
                vartable[str(l_node.value)] = Node(TokenType.INT, var_cnode.value)
            if (l_node.next == None):
                break
            l_node = l_node.next
            var_cnode = var_cnode.next

        m_node = stnode.value.next.next
        mo_node = m_node.value


        while True:
            if vartable.has_key(str(mo_node.value)):

                mo_node.type = vartable[mo_node.value].type
                mo_node.value = vartable[mo_node.value].value
            if mo_node.type ==5:
                if vartable.has_key(str(mo_node.value.value)):
                    mo_node.value.type = vartable[str(mo_node.value.value)].type
                    mo_node.value.value = vartable[str(mo_node.value.value)].value
            if (mo_node.next == None):
                break
            mo_node = mo_node.next
    ...
    if (op_code_node.type == 5):
        if (op_code_node.value.type == 21):
...
def Test_All():
    while True:
        ...
        if (definevalue != 1):
            print '...' + str(result)
        definevalue = 0
        vartable=copy.deepcopy(revartable)
