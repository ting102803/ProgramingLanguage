...
def run_func(op_code_node):
    ...
    def cdr(node):
        ...
        return create_new_quote_list(Node(TokenType.LIST,result.next))#리스트가 아닐경우 리스트를 만들어서 quote화한다
    ...
    def cons(node):
        ...
        """
        #Fill Out
        I_node = run_expr(node.value.next)
        temp = strip_quote(I_node.next).value
        result=Node(I_node.type,I_node.value)
        if result.type is TokenType.LIST:
            if result.value.type is TokenType.QUOTE:
                result=strip_quote(result);#리스트일경우 result를 노드화
        result.next=temp
        if result.type is not TokenType.LIST:
            return create_new_quote_list(result)
        return create_new_quote_list(Node(TokenType.LIST,result))
    ...
    def null_q(node):
        ...
        result = strip_quote(l_node).value
        if result is None:
            return Node(TokenType.TRUE,'T')
        return Node(TokenType.FALSE,'F')

    def atom_q(node):
        #Fill Out
        l_node = run_expr(node.value.next)
        if l_node.value.type is TokenType.QUOTE:
            if l_node.value.next.type is TokenType.LIST:
                if l_node.value.next.value is not None:
                    return Node(TokenType.FALSE,'F')
                else:
                    return Node(TokenType.TRUE, 'T')
            else:
